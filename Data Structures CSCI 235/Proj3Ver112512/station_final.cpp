...
bool Station::emptyQueues(int start, char direction)//return false if all four queues are empty
{
	bool flag=false;
	
	if(direction == 'n' || direction == 'N')
	{
		if(northQueue[start].getnOfPassengers()!=0 || northQueue[start+1].getnOfPassengers() != 0 || northQueue[start+2].getnOfPassengers() != 0 || northQueue[start+3].getnOfPassengers() != 0)
			return true;
		else
			return false;
	}
	...
	{
		if(southQueue[start].getnOfPassengers()!=0 || southQueue[start+1].getnOfPassengers() != 0 || southQueue[start+2].getnOfPassengers() != 0 || southQueue[start+3].getnOfPassengers() != 0)
			return true;
		else
			return false;
	}
	...
	if(direction=='s')
	{
		for(int i=start;i<lessThan;i++)
		{
			cout << "Testing emptyQueue: " << southQueue[i].getnOfPassengers() << endl;
...
void Station::fromQueueToTrain(char direction)
{
	...
	if(direction=='s')
	{
		...
		while(southTrain->getNumberInCar(1)!=64 && emptyQueues(0,'s'))
		{	
		...
		while(southTrain->getNumberInCar(2)!=64 && emptyQueues(4,'s'))
		{
		...
		while(southTrain->getNumberInCar(3)!=64 && emptyQueues(8,'s'))
		{
	...
	if(direction=='n')
	{
		...
		while(northTrain->getNumberInCar(1)!=64 && emptyQueues(0,'n'))
		{
		...
		////there is room on the care and there are// still passenger in the queu
		while(northTrain->getNumberInCar(2)!=64 && emptyQueues(4,'n'))
		{
			// if the queue is not empty dequeue the passanger into the train
		...
		while(northTrain->getNumberInCar(3)!=64 && emptyQueues(8,'n'))
		{
...
//Given a person variable of type Passenger, will sent to one of the random queues.
...
{
	northTrain=aTrain;
	//cout << stationName << " North Train Set!" << g_Min << endl;
}
...
{
	southTrain=aTrain;
	//cout << stationName << " South Train Set!" << g_Min << endl;
}
