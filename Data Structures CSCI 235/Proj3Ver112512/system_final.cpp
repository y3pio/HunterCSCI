...
void System::moveRedNorthTrains()//need to add from Train to queue and B3 transfer
{
	...
	if(nRTrack[11]!=NULL && g_Min-nRTrack[11]->getStartingTime()==18)
	{
		...
		A1.setSouthTrain(A1.getNorthTrain());
...
void System::moveTrains()
{
	...
	moveRedNorthTrains();
	moveRedSouthTrains();
	//write a loop here to increment all wait times
	/*A1.incrementWaitTime();
	A2.incrementWaitTime();
	A3.incrementWaitTime();
	A4.incrementWaitTime();
	A5.incrementWaitTime();
	B1.incrementWaitTime();
	B2.incrementWaitTime();
	B3R.incrementWaitTime();
	B3B.incrementWaitTime();
	B4.incrementWaitTime();
	B5.incrementWaitTime();*/
	...
	if(trainLine=='r' || trainLine=='R')
	{
		...
		do
		{
			...
			else if(transfer != NULL && transfer->destination=="A5")				//Else if case for stations south side.
			{
				...
				B3B.randomQueue('s', transfer);
			}
		}while(transfer!=NULL);//while there are transfer passengers at B3R on the direction line.//delete transfer after transferring passengers are done.
	}
